...
论文笔记放在 `_posts/` 里，和别的笔记一样，区别只在分类写两层：

```markdown
---
title: "MixFormer：Dense 与行为序列的联合扩展"
date: 2026-09-11 23:30:00 +0800
categories: [论文, 推荐系统]   # 第一个是「论文」，第二个是方向
tags: ["Co-scaling", "序列建模"]
description: >-
  一句话概括这篇论文做了什么。
math: true                    # 有公式就留这行，会加载 MathJax
---
```

写完之后，「分类」页会自动出现「论文」这张卡片，下面挂着「推荐系统」这样的子方向。以后再放别的方向的论文，把第二个分类换成新名字就行，比如 `categories: [论文, 多模态]`、`categories: [论文, LLM]`，不需要改任何模板。

公式写成独立的 `$$ ... $$` 段，页面会渲染成公式，不用截图。表格、代码块、引用都和普通笔记一样。

### 从本地批量导入

如果笔记已经在本地写好，可以用脚本导入，它会自动提标题、补分类、修掉指向本地文件的链接：
...
导入之后打开生成的文件，把 `description` 那句占位说明换成真正的摘要，再 `git push`。
